...
# 写入excel信息
write_excelFile = "/Users/weiyi/Desktop/test2.xlsx"
write_excelSheet = "Sheet2_sh"
...
desired_caps = {"platformName": "Android", "platformVersion": "10", "deviceName": "OnePlus2",
...
try:
    ...
    for value in publicIds:
        # 给输入框传值
        driver.find_element_by_id("com.tencent.mm:id/bhn").clear()
        time.sleep(2)
        ...
        if "没有更多的搜索结果" in source:
            ...
            if "不再关注" in source:
                ...
                if "操作过于频繁，请稍后再试。" in source:
                    ...
                    count += 1
                    # 将结果写入excel并且在控制台打印出来
                    write_toExcel(write_excelFile, write_excelSheet,
                                  construct_list(wechatName, value, count, "succeed"))
                    print("{} {} {} {}".format(wechatName, value, count, "succeed"))
